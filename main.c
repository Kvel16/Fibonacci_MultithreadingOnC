...
int main(int argc, char *argv[]) {
  ...
  stride = (int)(vl / nprocesadores); // numero de hilos
...
// esta es la funcion que se debe editar
void *fibonacci_over_vector(void *pos) {
  int thread_id = (int)pos; /*Posiciones de los hilos*/
  // int bajo = thread_id * stride;
  // int alto = (thread_id + 1) * stride;
  int contador;
  int i = (contador * stride) + thread_id;
